import React, { useContext, useState } from "react";
import { ImCross } from "react-icons/im";
import { VscDebugBreakpointLog } from "react-icons/vsc";
import { FaStar } from "react-icons/fa";
import toast, { Toaster } from "react-hot-toast";
import BgImage from "../../images/wavii2.jpg";
import { isAuthenticatedContext } from "../../context/userContext";
import axios from "axios";

const Syllabus = ({ topics, courseHeading }) => {
  const { user, setisAuthenticat } = useContext(isAuthenticatedContext);
  const [showFeedback, setShowFeedback] = useState(false);
  const [selectedTopic, setSelectedTopic] = useState(null);
  const [comment, setComment] = useState("");
  const [rating, setRating] = useState(0);
  const [selectedSubtopic, setSelectedSubtopic] = useState("");
  const [selectedSubtopics, setSelectedSubtopics] = useState([]);

  const handleFeedbackClick = (clickedTopic) => {
    setSelectedTopic(clickedTopic);
    setShowFeedback(true);
  };

  const handleDropdownChange = (event) => {
    setSelectedSubtopic(event.target.value);
    if (event.target.value && !selectedSubtopics.includes(event.target.value)) {
      setSelectedSubtopics([...selectedSubtopics, event.target.value]);
    }
  };

  const handleRemoveOption = (optionToRemove) => {
    setSelectedSubtopics(selectedSubtopics.filter((option) => option !== optionToRemove));
  };

  const handleStarClick = (value) => {
    setRating(value);
  };

  const handleCrossClick = () => {
    setShowFeedback(false);
    setSelectedTopic(null);
    setSelectedSubtopic("");
    setSelectedSubtopics([]);
    setComment("");
    setRating(0);
  };

  const datasave = async (e) => {
    e.preventDefault();
    // console.log("-------",selectedTopic.heading)
    if (!selectedTopic || selectedSubtopics.length === 0 || comment.trim() === "" || rating === 0) {
      toast.error("Please fill all the fields.");
      return;
    }

    const course = user.course;
    const email = user.email;
    const topic = selectedTopic.heading;
    try {
      const serverres = await axios.post(
        "http://localhost:5000/api/v1/feedback",
        {
          course,
          email,
          topic,
          selectedSubtopics,
          rating,
          comment,
        },
        {
          headers: {
            "Content-Type": "application/json",
          },
          withCredentials: true,
        }
      );

     console.log("result............",serverres)
     
      if (serverres.data.success && serverres.data.message === "Submitted successfully") {
       toast.success("Submitted successfully")
        setSelectedTopic(null);
        setSelectedSubtopic("");
        setSelectedSubtopics([]);
        setComment(""); 
        setRating(0);
        setShowFeedback(false);
      }
      //console.log("erorrrrr.......",error.response.data.message)
      if(error.response.data.message==="You have already filled"){
        toast.error("You have already filled")
        setSelectedTopic(null);
        setSelectedSubtopic("");
        setSelectedSubtopics([]);
        setComment(""); 
        setRating(0);
        setShowFeedback(false);
      }
    } catch (error) {
      console.error("Error:", error);
      // toast.error(error.response.data.message);
      if(error.response.data.message==="You have already filled"){
        toast.error("You have already filled")
        setSelectedTopic(null);
        setSelectedSubtopic("");
        setSelectedSubtopics([]);
        setComment(""); 
        setRating(0);
        setShowFeedback(false);
      }
      else{
        toast.error(error.response.data.message);
      }
    }
  };



  return (
    <>
      <section className="py-8">
        <div className="container mx-auto px-[80px] pb-10 flex flex-col items-center">
          <h1 className="text-[38px] md:text-[50px] font-bold pb-6 text-[#003366]">
            {courseHeading}
          </h1>
          <div className="flex bg-[#faead8] border-2 border-[#003366] shadow-lg shadow-[#003366] rounded-xl flex-wrap w-full items-center justify-center py-8">
            {topics.map((topic, index) => (
              <div key={index} className="flex flex-col md:flex-row w-[90%] justify-end py-2">
                <div className="w-full md:w-[70%]">
                  <h2 className="text-2xl font-bold pb-2 md:pb-0">{topic.heading}</h2>
                  <div className="flex flex-col md:flex-row flex-wrap pb-5 items-center">
                    {topic.subtopics.map((subtopic, subIndex) => (
                      <div key={subIndex} className="w-full md:w-[50%] flex gap-2 items-center">
                        <div className="">
                          <VscDebugBreakpointLog className="mr-[2px]" />
                        </div>
                        <div>
                          <h3>{subtopic}</h3>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                {user && user.profile === "Student" ? ( <div className="w-full md:w-[20%] text-center flex flex-col md:flex-row items-center justify-start">
                  <button
                    className="bg-[#003366] text-white font-semibold p-4 text-lg rounded-xl"
                    onClick={() => handleFeedbackClick(topic)}
                  >
                    Feedback
                  </button>
                </div> ) : ( 
                  <div className="w-full md:w-[20%] text-center flex flex-col md:flex-row items-center justify-start"> </div>
                 ) }

              </div>
            ))}
          </div>
        </div>
      </section>

      {showFeedback && (
        <div>
          <form onSubmit={datasave}>
            <div className="fixed inset-0 flex items-center justify-center">
              <div
                className="bg-black bg-opacity-50 absolute inset-0"
                onClick={handleCrossClick}
              ></div>
              <div className="bg-[#003366] w-[35%] relative rounded-md py-4 px-8 max-h-[80vh] overflow-auto">
                <button className="absolute top-[2%] right-2 text-black" onClick={handleCrossClick}>
                  <ImCross className="text-[#d19747]" />
                </button>
                <label className="block font-bold pb-2 text-[#d19747]">
                  Please specify the areas in which you're encountering difficulties:
                </label>
                <div>
                  <label className="block font-bold pb-2 text-[#d19747]">Topic:</label>
                  <input
                    type="text"
                    className="w-full px-3 py-2 border rounded-md mb-2"
                    value={selectedTopic ? selectedTopic.heading : ""}
                    readOnly
                  />
                </div>
                <div className="pb-2">
                  <label className="block font-bold pb-2 text-[#d19747]">Choose the subtopic:</label>
                  <div className="relative">
                    <select
                      className="w-full px-3 py-2 border rounded-md"
                      value={selectedSubtopic}
                      onChange={handleDropdownChange}
                    >
                      <option value="">Choose the subtopic</option>
                      {selectedTopic &&
                        selectedTopic.subtopics.map((subtopic, index) => (
                          <option key={index} value={subtopic}>
                            {subtopic}
                          </option>
                        ))}
                    </select>
                  </div>
                </div>
                <div className="addOptions">
                  {selectedSubtopics.map((option, index) => (
                    <div
                      key={index}
                      className="relative flex items-center bg-gray-200 my-1 px-2 py-1 rounded-md pr-1 pb-1"
                    >
                      <span className="pr-2">{option}</span>
                      <ImCross
                        className="absolute cursor-pointer right-1 text-[#d19747]"
                        onClick={() => handleRemoveOption(option)}
                      />
                    </div>
                  ))}
                </div>
                <div className="py-4">
                  <label className="block font-bold pb-2 text-[#d19747]">Rate this topic:</label>
                  <div className="flex items-center">
                    {[...Array(5)].map((_, index) => (
                      <FaStar
                        key={index}
                        className={
                          index < rating
                            ? "text-yellow-400 text-[30px] cursor-pointer mx-2"
                            : "text-gray-400 mx-2 cursor-pointer text-[30px]"
                        }
                        onClick={() => handleStarClick(index + 1)}
                      />
                    ))}
                  </div>
                </div>
                <div className="pb-4">
                  <label htmlFor="comment" className="block font-bold pb-2 text-[#d19747]">
                    Please provide any comments or suggestions to help us improve your understanding:
                  </label>
                  <textarea
                    id="comment"
                    rows="4"
                    className="w-full px-3 py-2 border rounded-md"
                    value={comment}
                    onChange={(e) => setComment(e.target.value)}
                  />
                </div>
                <div className="text-center pb-2">
                  <button className="bg-[#d19747] font-semibold text-white px-6 py-3 rounded-md">
                    Submit
                  </button>
                </div>
              </div>
              <Toaster position="top-center" reverseOrder={false} />
            </div>
          </form>
        </div>
      )}
    </>
  );
};

export default Syllabus;
